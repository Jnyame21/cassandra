...
<template>
  <div class="content-wrapper">
    <div id="subjectOverlay" class="overlay">
      <div class="info-container">
        ...
        <div class="flex-all-c ma-5 mt-15">
          ...
          <p class="title message" v-if="teacherName ==='none' ">No teacher has been assigned to teach this subject yet for the {{userAuthStore.activeAcademicYear}} academic year, {{userAuthStore.userData['academic_year']['period_division']['name']}} {{userAuthStore.activeTerm}}</p>
    ...
    <div v-if="userAuthStore.userData && userAuthStore.userData['current_yr'] !== 'COMPLETED' && userAuthStore.studentData && userAuthStore.studentData['subjects']"  class="subject-container">
      <v-card v-for="(subject, index) in userAuthStore.studentData.subjects" class="subject-card" @click="subjectInfo(subject['name'], subject['teacher'], subject['teacher_img'], subject['teacher_gender'], subject['teacher_email'], subject['teacher_contact'], subject['teacher_department'])" :key="index">
