...
<template>
    ...
    <div id="AdminAddAcademicYearOverlay" class="overlay" v-if="userAuthStore.adminData.classes?.length > 0">
      <div class="overlay-card">
        <v-btn @click="closeOverlay('AdminAddAcademicYearOverlay')" color="red" size="small" variant="flat" class="close-btn">X</v-btn>
        <p class="form-message" v-if="formErrorMessage" style="color: red">{{ formErrorMessage }}</p>
        <div class="overlay-card-info-container">
          <v-select class="select" :items="userAuthStore.adminData.classes.map((_class:any)=>_class['name'])" label="CLASS" v-model="selectedClass" 
          variant="solo-filled" density="comfortable" clearable
          />
          <div v-for="(_class, index) in userAuthStore.adminData.classes" :key="index">
            <v-select class="select" v-if="selectedClass === _class['name']" :items="_class['students'].map((_student:any)=>({name: _student['name'], st_id: _student['st_id']}))" label="REPEATED STUDENTS" v-model="selectedStudents" multiple chips
              item-title="name" item-value="st_id" variant="solo-filled" density="comfortable" clearable persistent-hint hint="Select the students who should be repeated">
        ...
        <div class="overlay-card-content-container">
          ...
          />
          <v-select class="select" :items="academicYearPeriodDivisionOptions" label="NUMBER OF DIVISIONS" v-model.number="academicYearPeriodDivisionSelected" 
          item-title="label" item-value="value" variant="solo-filled" density="comfortable" persistent-hint hint="How many division is in the academic year" 
          />
          <v-select class="select" v-if="academicYearPeriodDivisionSelected === 3" :items="term3DivisionOptions" label="DIVISION NAME" v-model="term3DivisionSelected" variant="solo-filled"
          item-title="label" item-value="value" density="comfortable" persistent-hint hint="Select the name given to each period" 
          />
          <v-text-field class="input-field" v-if="academicYearSelected" v-model="startDate" label="ACADEMIC YEAR START DATE" type="date"
            variant="solo-filled" density="comfortable" persistent-hint hint="Select the date the academic year starts" 
          />
          <v-text-field class="input-field" v-if="academicYearSelected" v-model="endDate" label="ACADEMIC YEAR END DATE" type="date"
            variant="solo-filled" density="comfortable" persistent-hint hint="Select the date the academic year ends" 
          />
          <v-text-field class="input-field" v-if="academicYearPeriodDivisionSelected === 2" v-model="term1EndDate" label="SEMESTER 1 END DATE" type="date"
            variant="solo-filled" density="comfortable" persistent-hint :hint="`Select the date semester 1 ends`" 
          />
          <v-text-field class="input-field" v-if="academicYearPeriodDivisionSelected === 2" v-model="term2StartDate" label="SEMESTER 2 START DATE" type="date"
            variant="solo-filled" density="comfortable" persistent-hint :hint="`Select the date semester 2 starts`" 
          />
          <v-text-field class="input-field" v-if="academicYearPeriodDivisionSelected === 2" v-model="term2EndDate" label="SEMESTER 2 END DATE" type="date"
            variant="solo-filled" density="comfortable" persistent-hint :hint="`Select the date semester 2 ends`" 
          />

          <v-text-field class="input-field" v-if="academicYearPeriodDivisionSelected === 3 && term3DivisionSelected" v-model="term1EndDate" :label="`${term3DivisionSelected} 1 END DATE`" type="date"
            variant="solo-filled" density="comfortable" persistent-hint :hint="`Select the date ${term3DivisionSelected} 1 ends`" 
          />
          <v-text-field class="input-field" v-if="academicYearPeriodDivisionSelected === 3 && term3DivisionSelected" v-model="term2StartDate" :label="`${term3DivisionSelected.toUpperCase()} 2 START DATE`" type="date"
            variant="solo-filled" density="comfortable" persistent-hint :hint="`Select the date ${term3DivisionSelected} 2 starts`" 
          />
          <v-text-field class="input-field" v-if="academicYearPeriodDivisionSelected === 3 && term3DivisionSelected" v-model="term2EndDate" :label="`${term3DivisionSelected.toUpperCase()} 2 END DATE`" type="date"
            variant="solo-filled" density="comfortable" persistent-hint :hint="`Select the date ${term3DivisionSelected} 2 ends`" 
          />
          <v-text-field class="input-field" v-if="academicYearPeriodDivisionSelected === 3 && term3DivisionSelected" v-model="term3StartDate" :label="`${term3DivisionSelected.toUpperCase()} 3 START DATE`" type="date"
            variant="solo-filled" density="comfortable" persistent-hint :hint="`Select the date ${term3DivisionSelected} 3 starts`" 
          />
          <v-text-field class="input-field" v-if="academicYearPeriodDivisionSelected === 3 && term3DivisionSelected" v-model="term3EndDate" :label="`${term3DivisionSelected.toUpperCase()} 3 END DATE`" type="date"
            variant="solo-filled" density="comfortable" persistent-hint :hint="`Select the date ${term3DivisionSelected} 3 ends`" 
          />
          <v-text-field class="input-field" v-if="academicYearSelected" v-model="graduationDate" label="GRADUATION DATE(OPTIONAL)" type="date"
            variant="solo-filled" density="comfortable" persistent-hint hint="Select the date students will graduate" 
          />
        </div>
        <div class="overlay-card-action-btn-container">
          <v-btn @click="createAcademicYear()" :disabled="checkInput" :ripple="false" variant="flat"
    ...
    <div class="content-header">
      <v-btn class="ma-1" color="blue"  @click="showOverlay('AdminAddAcademicYearOverlay')" :size="elementsStore.btnSize1">
        ADD ACADEMIC YEAR
      </v-btn>
    </div>
      <TheLoader v-if="!userAuthStore.adminData.academicYears" />
      <v-table fixed-header class="table" v-if="userAuthStore.adminData.academicYears?.length">
        ...
        <tr>
          ...
          <th class="table-head" v-if="!userAuthStore.userData['school']?.['semesters']">{{ userAuthStore.userData['academic_year']['period_division']['name'] }} 3 START DATE</th>
          <th class="table-head" v-if="!userAuthStore.userData['school']?.['semesters']">{{ userAuthStore.userData['academic_year']['period_division']['name'] }} 3 END DATE</th>
        ...
        <tr v-for="(_year, index) in userAuthStore.adminData.academicYears" :key="index">
          ...
          <td class="table-data" v-if="!userAuthStore.userData['school']?.['semesters']">{{_year['term_3_start_date']}}</td>
          <td class="table-data" v-if="!userAuthStore.userData['school']?.['semesters']">{{_year['term_3_end_date']}}</td>
          <td class="table-data">
            <v-btn color="red" size="x-small" variant="flat" v-if="index === 0" @click="elementsStore.ShowDeletionOverlay(()=>deleteAcademicYear(), 'Are you sure you want to delete this academic year?')">
              DELETE
            </v-btn>
          </td>
        </tr>
